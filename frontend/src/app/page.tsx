...
const RigCard = ({ rig }: { rig: Rig }) => {
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    e.preventDefault();
    if (confirm(`确定要删除台架 ${rig.rig_id} 吗？此操作不可撤销。`)) {
      try {
        const res = await fetch(`${CONFIG.API_BASE_URL}/api/status/${rig.rig_id}`, {
          method: 'DELETE',
        });
        if (res.ok) {
          // 重新获取数据以更新界面
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        ...
        <div className="text-right flex flex-col items-end gap-2">
           <button 
             ...
             className="opacity-0 group-hover:opacity-100 transition-all p-1.5 hover:bg-rose-500/10 rounded-lg text-zinc-600 hover:text-rose-500"
             title="Delete Rig"
           >
